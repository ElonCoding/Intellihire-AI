...
export default function Login() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Optional: Store in Firestore if new user
        ...
        <div className="p-8 rounded-3xl bg-white/5 border border-white/10 glassmorphism relative overflow-hidden">
          ...
          <div className="space-y-4 mb-6">
            <button 
              ...
              className="w-full py-3 px-4 flex items-center justify-center gap-3 rounded-xl bg-white text-black font-medium hover:bg-white/90 transition-colors"
            >
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            {!isLogin && (
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" 
                />
              </div>
            )}
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 pl-11 text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" 
                  required
                />
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full py-3 rounded-xl bg-primary hover:bg-primary/90 text-white font-medium flex items-center justify-center gap-2 transition-all mt-6 shadow-[0_0_15px_rgba(59,130,246,0.3)] disabled:opacity-50"
            >
